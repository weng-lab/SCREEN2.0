...
>({
  ...
  const [sampleTypeFilter, setSampleTypeFilter] =
    useState<SampleTypeCheckboxes>({
      "Cell Line": true,
      "Primary Cell": true,
      Tissue: true,
      Organoid: true,
  ...
  // Use refs to track page updates without causing re-renders
  const pageUpdateTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const pendingPageUpdatesRef = useRef<{ [key: string]: number }>({});

  const handlePageChange = useCallback((ontology: string, newPage: number) => {
    // Store the update in the ref
    pendingPageUpdatesRef.current[ontology] = newPage;
    ...
    // Set a new timeout to apply the updates after render
    pageUpdateTimeoutRef.current = setTimeout(() => {
      const updates = { ...pendingPageUpdatesRef.current };
      pendingPageUpdatesRef.current = {};

      setPageStates((prev) => ({ ...prev, ...updates }));
      pageUpdateTimeoutRef.current = null;
    }, 0);
  }, []);
   ...
   */
  const unfilteredBiosamples: { [key: string]: BiosampleData<HasRNASeq>[] } =
    useMemo(() => {
    ...
    if (allowMultiSelect) {
      (selected as string[]).forEach((x) => {
        ...
        if (match) {
          ...
          console.error(
            `Could not find biosample with name or displayname: ${x}`
          );
        }
      });
    } else {
      foundMatches.push(
        data.find(
          (sample) =>
            sample.name === selected || sample.displayname === selected
        )
      );
    }
  ...
  const biosampleTables = useMemo(() => {
      ...
      colsToSpread.push(
        {
          ...
          render: (row) => (
            <DownloadButton row={row} downloadType="celltypeccres" />
          ),
        },
        {
          header: "DNase Signal",
    ...
    if (errorBiosamples || error_rnaseq) {
      return (
        <Typography>
          Something went wrong fetching biosamples, check the console for more
          information.
        </Typography>
      );
    }
    ...
    const handleModifyAll = (
      samples: BiosampleData<HasRNASeq>[],
      action: "select" | "deselect"
    ) => {
      console.log("modifying all");
      if (onChange && typeof onChange === "function") {
        if (action === "select") {
          const toAdd = samples.filter(
            (sample) => !selectedSamples.some((x) => x.name === sample.name)
          );
          (onChange as (selected: BiosampleData<HasRNASeq>[]) => void)([
            ...selectedSamples,
            ...toAdd,
          ...
          const toRemove = samples.filter((sample) =>
            selectedSamples.some((x) => x.name === sample.name)
          );
          (onChange as (selected: BiosampleData<HasRNASeq>[]) => void)(
    ...
    return Object.entries(filteredBiosamples)
      ...
      .map(([ontology, biosamples]) => {
        ...
        const selectAllChecked = biosamples.every((sample) =>
          selectedSamples.some((x) => x.name === sample.name)
        );
        const selectAllIndeterminate =
          biosamples.some((sample) =>
            selectedSamples.some((x) => x.name === sample.name)
          ) && !selectAllChecked;
            ...
            HeaderRender: () => (
              <Checkbox
                ...
                onClick={(e) => {
                  ...
                  handleModifyAll(
                    biosamples,
                    selectAllChecked ? "deselect" : "select"
                  );
                }}
              />
            ),
            ...
            <AccordionDetails>
              <DataTable
                ...
                onDisplayedRowsChange={(newPage) => {
                  if (
                    pageStates[ontology] === undefined ||
                    pageStates[ontology] !== newPage
                  ) {
                    handlePageChange(ontology, newPage);
                  }
                }}
                columns={columns}
                rows={biosamples}
                ...
                sortColumn={1}
                {...(!showCheckboxes && { onRowClick: handleRowClick })}
              />
            </AccordionDetails>
          </Accordion>
        );
  ...
  const handleResetFilters = useCallback(() => {
    ...
    handleSetLifeStageFilter(
      Object.fromEntries(
        Object.entries(lifeStageFilter).map(([key]) => [key, true])
      ) as LifeStageCheckboxes
    );
    handleSetSampleTypeFilter(
      Object.fromEntries(
        Object.entries(sampleTypeFilter).map(([key]) => [key, true])
      ) as SampleTypeCheckboxes
    );
    handleSetCollectionFilter(
      Object.fromEntries(
        Object.entries(collectionFilter).map(([key]) => [key, true])
      ) as CollectionCheckboxes
    );
                ...
                <FormControlLabel
                  ...
                  control={
                    <Checkbox
                      ...
                      onChange={(e) =>
                        handleSetMustHaveRnaSeq(e.target.checked)
                      }
                    />
                  }
                />
