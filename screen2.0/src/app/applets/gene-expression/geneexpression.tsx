...
import DownloadDialog, {
  FileOption,
} from "../gwas/_lollipop-plot/DownloadDialog";
import {
  capitalizeFirstLetter,
  downloadObjArrayAsTSV,
  downloadSVG,
  downloadSvgAsPng,
} from "../gwas/helpers";
...
const allBiosampleTypes: BiosampleType[] = [
  "cell line",
  "primary cell",
  "tissue",
  "in vitro differentiated cells",
...
type GeneDataset = {
  ...
  accession: string;
  gene_quantification_files?: Array<{
    __typename?: "GeneQuantificationFile";
    accession: string;
    biorep?: number | null;
    quantifications?: Array<{
...
export function GeneExpression(props: {
  ...
  const initialAssembly =
    urlAssembly === "GRCh38" || urlAssembly === "mm10"
      ? urlAssembly
      : props.assembly;
  ...
  const [gene, setGene] = useState<string>(
    props.genes ? props?.genes[0]?.name : urlGene ?? "APOE"
  );
  const [assembly, setAssembly] = useState<Assembly>(initialAssembly);
  const [biosamples, setBiosamples] =
    useState<BiosampleType[]>(allBiosampleTypes);
  ...
  const [viewBy, setViewBy] = useState<
    "byTissueMaxTPM" | "byExperimentTPM" | "byTissueTPM"
  >("byExperimentTPM");
  const [RNAtype, setRNAType] = useState<
    "all" | "polyA plus RNA-seq" | "total RNA-seq"
  >("total RNA-seq");
  ...
  const sampleMatchesSearch = useCallback(
    (x: GeneDataset): boolean => {
      if (search) {
        return (
          x.accession.toLowerCase().includes(search.toLowerCase()) ||
          x.biosample.toLowerCase().includes(search.toLowerCase()) ||
          x.tissue.toLowerCase().includes(search.toLowerCase()) ||
          !!x.gene_quantification_files
            .map((x) => x.accession)
            .find((y) => y.toLowerCase().includes(search.toLowerCase()))
        );
      } else return true;
    },
    [search]
  ...
  const { data: dataGeneID, loading: loadingGeneID } = useQuery(GENE_QUERY, {
    variables: {
  ...
  //Fetch Gene Exp data
  const { data: dataExperiments, loading: loadingExperiments } = useQuery(
    GENE_EXP_QUERY,
    {
      variables: {
        assembly: assembly,
        gene_id:
          dataGeneID &&
          dataGeneID.gene.length > 0 &&
          dataGeneID.gene[0].id.split(".")[0],
      },
      skip:
        !gene || !dataGeneID || (dataGeneID && dataGeneID.gene.length === 0),
      fetchPolicy: "cache-and-network",
      nextFetchPolicy: "cache-first",
      onCompleted(data) {
        ...
        data.gene_dataset.forEach((x) => {
          ...
          if (!existingEntry) {
            // if tissue not cataloged, add to set
            tissues.push({
              label: x.tissue,
              types: [x.biosample_type as BiosampleType],
            });
          } else if (
            !existingEntry.types.includes(x.biosample_type as BiosampleType)
          ) {
            //if tissue cataloged, but an exisitng biosample type doesn't, add biosample type
  ...
  const {
    ...
    error: errorOrtholog,
  } = useQuery<GET_ORTHOLOG_DATA, GET_ORTHOLOG_VARS>(GET_ORTHOLOG, {
    variables: {
      name: [gene],
  ...
  const makeLabel = (
    ...
    const name = capitalizeFirstLetter(
      biosample.includes(",") ? biosample.split(",")[0] + "..." : biosample
    );
    return `${tpm.toFixed(2)}, ${name} (${accession}${
      biorep ? ", rep. " + biorep : ""
    })`;
  };
    ...
    if (dataExperiments && dataExperiments.gene_dataset.length > 0) {
      ...
      switch (viewBy) {
        ...
        case "byTissueTPM": {
          ...
          const maxValuesByTissue: { [key: string]: number } =
            parsedReplicates.reduce((acc, item) => {
              acc[item.category] = Math.max(
                acc[item.category] || -Infinity,
                item.value
              );
              return acc;
            }, {});
          ...
          parsedReplicates.sort((a, b) => {
            const maxDiff =
              maxValuesByTissue[b.category] - maxValuesByTissue[a.category];
        ...
        case "byTissueMaxTPM": {
          ...
          parsedReplicates = parsedReplicates.filter(
            (x) => x.value === maxValuesByTissue[x.category]
          );
          parsedReplicates.sort((a, b) => b.value - a.value);
        }
      }
  ...
  const handleSetAssembly = (newAssembly: Assembly) => {
    if (props.applet) {
      //only allow switch in applet. Check should never fail but keeping
  ...
  const handleGroupChange = (
    ...
    if (
      newView !== null &&
      (newView === "byTissueMaxTPM" ||
        newView === "byExperimentTPM" ||
        newView === "byTissueTPM")
    ) {
      setViewBy(newView);
    }
  ...
  const handleRNATypeChange = (
    ...
    if (
      newRNA !== null &&
      (newRNA === "all" ||
        newRNA === "polyA plus RNA-seq" ||
        newRNA === "total RNA-seq")
    ) {
      setRNAType(newRNA);
    }
  };
  ...
  const handleScaleChange = (
    ...
    if (
      newScale !== null &&
      (newScale === "linearTPM" || newScale === "logTPM")
    ) {
      setScale(newScale);
    }
  };
  ...
  const handleReplicatesChange = (
    ...
    if (
      newReplicates !== null &&
      (newReplicates === "mean" || newReplicates === "all")
    ) {
  ...
  const handleDownload = useCallback(
    (selectedOptions: FileOption[]) => {
      if (selectedOptions.includes("svg")) {
        downloadSVG(plotRef, gene + "_gene_expression");
      }
      if (selectedOptions.includes("png")) {
        downloadSvgAsPng(plotRef, gene + "_gene_expression");
      }
        ...
        const toDownload: {
          ...
          .map((x) => {
            return {
              ...
              tissue: x.tissue,
              tpm: (
                x.gene_quantification_files.reduce(
                  (acc, cur) => acc + cur.quantifications?.[0]?.tpm,
                  0
                ) / x.gene_quantification_files.length
              ).toFixed(2),
            };
          })
    ...
    (bar: BarData<GeneDataset>) => {
      return (
        <Box maxWidth={350}>
          ...
          <br />
          <Typography variant="body2">
            <b>Accession:</b> {bar.metadata.accession}
          </Typography>
          <Typography variant="body2">
  ...
  const handleSetTissues: MultiSelectOnChange<MultiSelectTissue> = (
    _,
    value
  ) => {
  ...
  const handleSetBiosamples: MultiSelectOnChange<BiosampleType> = (
    _,
    value
  ) => {
    setBiosamples(value);
  ...
  const resetTissues = () => [setSelectedTissues(availableTissues)];
  ...
  return (
    <Stack spacing={2}>
      <Stack direction="row" justifyContent={"space-between"}>
        <Typography alignSelf={"flex-end"} variant={props.applet ? "h4" : "h5"}>
        ...
        <Stack direction="row" spacing={3}>
          ...
          >
            <Image
              ...
              src="https://genome-euro.ucsc.edu/images/ucscHelixLogo.png"
              fill
              alt="ucsc-button"
            />
          </Button>
          ...
          >
            <Image
              style={{ objectFit: "contain" }}
              src="https://geneanalytics.genecards.org/media/81632/gc.png"
              fill
              alt="gene-card-button"
            />
          </Button>
        </Stack>
      </Stack>
        ...
        <Stack direction="row" gap={2} flexWrap={"wrap"}>
          ...
          <GeneAutocomplete
            ...
            onGeneSelected={(gene: GeneInfo) => {
              ...
              router.push(
                `${pathname}?assembly=${assembly}&gene=${gene ? gene.name : ""}`
              );
            }}
          ...
          <LoadingButton
            ...
            onClick={() => {
              const newGene =
                dataOrtholog.geneOrthologQuery[0][
                  assembly === "GRCh38" ? "mouseGene" : "humanGene"
                ];
              const newAssembly = assembly === "GRCh38" ? "mm10" : "GRCh38";
              handleSetGene(newGene);
              handleSetAssembly(newAssembly);
              router.push(
                `${pathname}?assembly=${newAssembly}&gene=${newGene}`
              );
            }}
          >
            {dataOrtholog?.geneOrthologQuery.length > 0 ? (
              <>
                {`Go to ${assembly === "GRCh38" ? "mm10" : "GRCh38"} ortholog:`}
                &thinsp;
                <i>
                  {
                    dataOrtholog?.geneOrthologQuery[0][
              ...
              `No ortholog found in ${
                assembly === "GRCh38" ? "mm10" : "GRCh38"
              }`
            )}
          </LoadingButton>
        </Stack>
      ...
      <Stack direction="row" gap={2} flexWrap={"wrap"}>
        <FormControl>
          ...
          >
            ...
            <Tooltip title={assembly === "GRCh38" && "Only available in mm10"}>
              <div>
                {" "}
                {/** div needed to show tooltip when button disabled */}
                <ToggleButton
        ...
        <FormControl>
          <FormLabel>
            {biosamples.length === allBiosampleTypes.length ? (
              "Biosample Types"
            ) : (
              <Stack direction="row" justifyContent={"space-between"}>
                <i>Biosample Types*</i>
          ...
          <MultiSelect
            ...
            getOptionDisabled={(option) =>
              !selectedTissues.some((x) => x.types.includes(option))
            }
        ...
        <FormControl>
          <FormLabel>
            {selectedTissues.length === availableTissues.length ? (
      ...
      <Stack direction={"row"} gap={2} flexWrap={"wrap"}>
        <FormControl sx={{ minWidth: 150 }}>
          <InputLabel size="small" style={{ zIndex: 0 }}>
            Search Samples
          </InputLabel>
          <OutlinedInput
            size="small"
      ...
      {loadingGeneID || loadingExperiments ? (
        ...
        <VerticalBarPlot
          ...
          topAxisLabel={
            ...
            assembly +
            " - " +
            (scale === "linearTPM" ? "Linear TPM" : "Log10(TPM + 1)")
          }
          SVGref={plotRef}
          onBarClicked={(x) =>
            window.open(
