    ...
    <g transform={transform}>
      <DenseBigBed
        ...
        tooltipContent={(rect) => (
          ...
          >
            <b>
              {stateDetails[rect.name].description}
              {"(" + stateDetails[rect.name].stateno + ")"}
            </b>
            {"\n"}
            {rect.name}
            <br />
...
export const HumanChromHmmTracks: React.FC<HumanChromHmmTracksProps> = (
  props
) => {
  const [cTracks, setTracks] = useState<[string, string, string][]>(
    props.tracks
  );
