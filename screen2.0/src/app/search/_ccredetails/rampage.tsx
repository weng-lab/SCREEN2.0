    ...
    loading ?
      ...
      data && data.length == 0 ? (<Typography>No data available</Typography>) :
        <Grid2 container spacing={2}>
          ...
          <Grid2 xs={12}>
            <Typography>
              {data_gene?.gene[0] && "Gene ID: " + data_gene.gene[0].id + " (" + peakDetails?.locusType + ")"}
            </Typography>
            <Typography>
              {`${peak}: ${peakDetails?.chrom + ":" + peakDetails?.start.toLocaleString() + "-" + peakDetails?.end.toLocaleString()}`}
            </Typography>
          </Grid2>
          <Grid2 xs={12}>
