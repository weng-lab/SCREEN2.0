...
export const Browser = ({
  ...
  useEffect(() => {
    const bigbeds = state.tracks.filter(
      (track) => track.trackType === TrackType.BIGBED
    );
    ...
    const transcripts = state.tracks.filter(
      (track) => track.trackType === TrackType.TRANSCRIPT
    );
  ...
  const biosampleIDs = useMemo(() => {
    return state.tracks
      .filter((track) => track.id.startsWith("sample-"))
      .map((track) => track.id);
  }, [state.tracks]);
  ...
  useEffect(() => {
    ...
    if (biosampleIDs.length > 0) {
      const currentSample =
        state.tracks[state.tracks.length - 1].id.split("-")[2];
    ...
    >
      ...
      >
        <h3 style={{ marginBottom: "0px", marginTop: "0px" }}>
          {coordinates.assembly} at {state.domain.chromosome}:
          {state.domain.start.toLocaleString()}-
          {state.domain.end.toLocaleString()}
        </h3>
        <svg id="cytobands" width={"700px"} height={20}>
          <GQLCytobands
            assembly={coordinates.assembly === "GRCh38" ? "hg38" : "mm10"}
