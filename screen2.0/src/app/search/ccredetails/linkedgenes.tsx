...
export const LinkedGenes = ({accession,assembly}) =>{
    // returns geneids from linked genes query
  const geneIDs = (linkedGenes: {accession: string,assay: string,assembly: string,
    celltype: string, experiment_accession: string,gene: string}[]) => {
  ...
  if(data_linked && data_genes){
    ...
    let linked: geneRow[]  = [], ids:  { [key: string]: string | undefined } = {}
    for (let x of linkedgeneIDs)
        ids[x.id.split(".")[0]] = x.name
    ...
    {(loading_genes || loading_linked) || (!linked_genes) ? 
        ...
        <Grid2 container spacing={3} sx={{ mt: "1rem", mb: "2rem" }}>
            <Grid2 xs={12} lg={12}>        
                ...
                columns={
                    [
                        { header: "Gene", value: (row: geneRow) => row.gene, render: (row: geneRow) => createLink("http://www.genecards.org/cgi-bin/carddisp.pl?gene=", row.gene) },
                        {
                            header: "Biosample",
