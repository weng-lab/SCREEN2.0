...
const genesDistance = (coordinates: {chromosome: string, start: number, end: number}, ccrecoord: {chromosome: string, start: number, end: number}) => {
  let c = Math.floor((ccrecoord.start + ccrecoord.end) / 2)
  return Math.min(Math.abs(coordinates.start - c), Math.abs(coordinates.end - c))
}

const cCREDistance = (coordinates: {chromosome: string, start: number, end: number}, ccrecoord: {chromosome: string, start: number, end: number}) => {
  let c = Math.floor((ccrecoord.start + ccrecoord.end) / 2)
...
const snpDistance = (coordinates: {chromosome: string, start: number, end: number}, ccrecoord: {chromosome: string, start: number, end: number}) => {
  let c= Math.floor((ccrecoord.start + ccrecoord.end) / 2)
  return Math.abs(c - coordinates.start)
  
}
export const NearByGenomicFeatures: React.FC<{assembly: string, accession: string, coordinates: {chromosome: string, start: number, end: number}}> = ({assembly, accession, coordinates})=>{
    ...
    let genes = data && data.gene && data.gene.map((g=>{
      ...
      let ccres = data && data.cCREQuery && data.cCREQuery.map(c=>{
        return {
            name: c.accession,
            distance: cCREDistance(c.coordinates, coordinates)
        }
      })
      let snps = data &&  data.snpQuery && data.snpQuery.map(s=>{
        return {
            assembly,
            accession: accession,
                  ...
                  columns={[
                    {
                      header: "SNP Id",
                      value: (row) => row.name,
                      render: (row) => createLink(row.assembly.toLowerCase()==="grch38"? "http://ensembl.org/Homo_sapiens/Variation/Explore?vdb=variation;v=":  "http://ensembl.org/Mus_musculus/Variation/Explore?vdb=variation;v=", row.name ),
                    },
                    {
                      header: "Distance",
                      value: (row) => row.distance,
