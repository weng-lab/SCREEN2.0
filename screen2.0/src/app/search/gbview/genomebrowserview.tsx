...
export const GenomeBrowserView: React.FC<GenomeBrowserViewProps> = (props) =>{
  console.log(props.gene)
    ...
    useEffect(()=>{
        ...
        const result =  props.assembly==="mm10" ?  mouseBiosamples.find(m=>m.name===props.biosample) : humanBiosamples.find(m=>m.name===props.biosample)
        const r = [result.dnase_signal, result.h3k4me3_signal, result.h3k27ac_signal, result.ctcf_signal].filter((x) => !!x)
            ...
            if(result.dnase_signal) tracks.push([`DNase-seq signal in ${props.biosample}`,`https://www.encodeproject.org/files/${result.dnase_signal}/@@download/${result.dnase_signal}.bigWig`,"DNase"])
            if(result.h3k4me3_signal) tracks.push([`H3K4me3 ChIP-seq signal in ${props.biosample}`,`https://www.encodeproject.org/files/${result.h3k4me3_signal}/@@download/${result.h3k4me3_signal}.bigWig`,"H3K4me3"])
            if(result.h3k27ac_signal) tracks.push([`H3K27ac ChIP-seq signal in ${props.biosample}`,`https://www.encodeproject.org/files/${result.h3k27ac_signal}/@@download/${result.h3k27ac_signal}.bigWig`,"H3K27ac"])
            if(result.ctcf_signal) tracks.push([`CTCF ChIP-seq signal in ${props.biosample}`,`https://www.encodeproject.org/files/${result.ctcf_signal}/@@download/${result.ctcf_signal}.bigWig`,"CTCF"])
        setTracks(tracks)
    ...
    <br/>
      ...
      >
        ...
        />
        {props.biosample && props.assembly!="mm10" && cTracks && <BiosampleTracks
          assembly={props.assembly}
