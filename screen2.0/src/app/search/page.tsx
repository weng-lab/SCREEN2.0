...
export default function Search({
  ...
  const handlecCREClick = useCallback(
    (item) => {
      const newcCRE = {
        ...
        region: { start: item.start, end: item.end, chrom: item.chromosome },
      };
      const color =
        item.color ||
  ...
  useEffect(() => {
    ...
    const getcCREs = async () => {
      const accessions = mainQueryParams.searchConfig.bed_intersect
        ...
        >
          ...
          {page < numberOfDefaultTabs ? (
            ...
            >
              ...
              />
              {mainQueryParams.coordinates.assembly !== "mm10" && (
                <StyledVerticalTab value={10} label="ENTEx" />
              )}
            </Tabs>
          )}
        </Drawer>
