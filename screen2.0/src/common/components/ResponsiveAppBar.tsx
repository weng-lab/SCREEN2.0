...
function ResponsiveAppBar() {
  ...
  const handleOpenNavMenu_Dropdown = (event: React.MouseEvent<HTMLElement>, dropdownID: string) => {
    ...
    if (dropdownID == '0') { setAnchorElNav_Dropdown0(event.currentTarget); }
    else if (dropdownID == '1') { setAnchorElNav_Dropdown1(event.currentTarget); }
  };
  ...
  const handleCloseNavMenu_Dropdown = (dropdownID: string) => {
    ...
    if (dropdownID == '0') { setAnchorElNav_Dropdown0(null); }
    else if (dropdownID == '1') { setAnchorElNav_Dropdown1(null); }
  };
          ...
          <Box sx={{ flexGrow: 0, display: { xs: 'inline', md: 'none' } }}>
            ...
            >
              <MenuItem onClick={handleCloseNavMenu_Hamburger}>
                <Typography component='a' href={`${nextConfig.basePath}`} textAlign="center">Home</Typography>
              </MenuItem>
              {pageLinks.map((page) => (
                  ...
                  >
                    {page.subPages && page.subPages.map((subPage) => (
                      <MenuItem key={subPage.pageName} onClick={() => handleCloseNavMenu_Dropdown(page.dropdownID)}>
                        {/* Wrap in next/link to enable dyanic link changing from basePath in next.config.js */}
                        <Link href={subPage.link}><Typography textAlign="center">{subPage.pageName}</Typography></Link>
                      </MenuItem>
                    ))}
                  </Menu>
                }
