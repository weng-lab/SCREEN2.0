    ...
    <Grid container sx={{ mr: "1em", ml:"1em" }}>      
      <Grid item sm={5.5} md={5.5} lg={5.5} xl={5.5}>
        <Autocomplete
          ...
          onKeyDown={(event) => {
            if (event.key === "Enter") {
              ...
              if (value) {
                ...
                router.push(`search?assembly=${props.assembly}&chromosome=${chromosome}&start=${Math.max(0,start)}&end=${end}&snpid=${value}`)
              }
            }
          }}
          value={value}
          ...
          renderInput={(params) => (
            <TextField {...params}  label="Enter a snp rsId" InputLabelProps={{ shrink: true }}  placeholder="e.g. rs11669173" fullWidth />
          )}
